...
router.route('/results/').get(async(req,res) =>{
    ...
    if(!page){
     ...
     if (page){
      ...
      const resultsdb = mongoose.connection.useDb("hupoxnet_results")
...
// router.route('/network_domain/').get(async(req,res) =>{
//   const body = JSON.parse(JSON.stringify(req.body));
...
//     let final = await Results.find().limit(5000).exec()
//     let counts = await Results.count()
//     let host_protein =await Results.distinct("Host_Protein")
...
router.route('/drugs/').get(async(req, res) => {
  ...
  let {species, page, size} = req.query
    if(!page){
     ...
     if (page){
      ...
      let drugs_results = await Drugs[species].find().limit(limit).skip(skip).exec()
      let total = await Drugs[species].count()

      res.json({'data':drugs_results, 'total':total})
